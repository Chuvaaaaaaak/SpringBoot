...
function valid() {
    ...
    for (let it of rez) {
        if ($('#username').val() === it) {
            $('#username').after('<span class="error alert-danger">Такой логин уже существует</span>');
            flag = false;
        }
    }
...
function validModal() {
    ...
    // if (checkUsername($('#usernameEd').val()) === false) {
    //     $('#usernameEd').after('<span class="error alert-danger">Оригинал почты</span>');
    //     flagValid = false;
    // }
